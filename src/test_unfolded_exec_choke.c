...
enum { ITERS = 100000 };
enum { CELL_SIZE = 11, NUM_CELLS = 3, CODE_SIZE = NUM_CELLS * CELL_SIZE + 1 };
...
static uint8_t *build_chain_region(void) {
    ...
    for (int i = 0; i < NUM_CELLS; ++i) {
        ...
        buf[base + 0] = 0x0F; buf[base + 1] = 0xBD; buf[base + 2] = 0xC0; // 1x BSR
        buf[base + 3] = 0xF0; buf[base + 4] = 0x0F; buf[base + 5] = 0xB0; buf[base + 6] = 0x15; // lock cmpxchg [rip+disp32],dl
        size_t target = (size_t)((i + 1) % NUM_CELLS) * CELL_SIZE;
        int32_t disp = (int32_t)(target - (base + CELL_SIZE));
        memcpy(&buf[base + 7], &disp, sizeof(disp));
    }
    buf[NUM_CELLS * CELL_SIZE] = 0xC3;
...
int main(void) {
    ...
    run_case((cell_fn_t)shared, (cell_fn_t)shared, shared0, shared1);
    run_case((cell_fn_t)sep0, (cell_fn_t)sep1, sep0d, sep1d);
    ...
    printf("shared   mean0=%.9f mean1=%.9f\n", m_shared0, m_shared1);
    printf("separate mean0=%.9f mean1=%.9f\n", m_sep0, m_sep1);
    printf("ratio_sep_shared_core0=%.12f\n", m_sep0 / m_shared0);
    printf("ratio_sep_shared_core1=%.12f\n", m_sep1 / m_shared1);
    printf("ratio_shared_sep_core0=%.12f\n", m_shared0 / m_sep0);
    printf("ratio_shared_sep_core1=%.12f\n", m_shared1 / m_sep1);

    write_csv("/mnt/data/unfolded_shared_core0.csv", shared0);
    write_csv("/mnt/data/unfolded_shared_core1.csv", shared1);
    write_csv("/mnt/data/unfolded_separate_core0.csv", sep0d);
    write_csv("/mnt/data/unfolded_separate_core1.csv", sep1d);

    FILE *sum = fopen("/mnt/data/unfolded_summary.txt", "w");
    ...
    fprintf(sum, "ratio_shared_sep_core0=%.12f\n", m_shared0 / m_sep0);
    fprintf(sum, "ratio_shared_sep_core1=%.12f\n", m_shared1 / m_sep1);
    fclose(sum);
    return 0;
}
