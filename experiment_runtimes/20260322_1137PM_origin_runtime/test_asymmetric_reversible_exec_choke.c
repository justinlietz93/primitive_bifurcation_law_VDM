...
static region_t build_region(void) {
    region_t r = {0};
    r.cell_size = 3 + 3 + 10 + 2 + 2;
    r.code_size = NUM_CELLS * r.cell_size + 1;
    r.mutable_off = 3 + 3 + 10 + 2;
    ...
    for (int i = 0; i < NUM_CELLS; ++i) {
        ...
        r.buf[p++] = 0x66; r.buf[p++] = 0xF0; r.buf[p++] = 0x81; r.buf[p++] = 0x35; // lock xor word ptr [rip+disp32], imm16
        ...
        r.buf[p++] = 0xF7; r.buf[p++] = 0x10; // imm16 = 0x10F7 toggles 00 C0 <-> F7 D0
        r.buf[p++] = 0x0F; r.buf[p++] = 0xA2; // cpuid serialize before executing mutable pair
        r.buf[p++] = 0x00; r.buf[p++] = 0xC0; // mutable successor starts at add al,al
    }
...
static void dump_pairs(FILE *f, const char *label, const region_t *r) {
    fprintf(f, "%s", label);
    for (int i = 0; i < NUM_CELLS; ++i) {
        int off = i * r->cell_size + r->mutable_off;
        fprintf(f, " %02x%02x", r->buf[off], r->buf[off + 1]);
    }
    fprintf(f, "\n");
}

static void trace_passes(const char *path) {
    region_t tr = build_region();
    FILE *f = fopen(path, "w");
    if (!f) { perror(path); exit(1); }
    dump_pairs(f, "initial:", &tr);
    for (int p = 1; p <= 8; ++p) {
        asm volatile(
            "xor %%eax, %%eax\n\t"
            "call *%0\n\t"
        ...
        dump_pairs(f, label, &tr);
    }
    fclose(f);
}

int main(void) {
    FILE *notes = fopen("/mnt/data/asymmetric_reversible_exec_choke_notes.md", "w");
    if (!notes) { perror("notes"); return 1; }
    fprintf(notes,
"# Asymmetric Reversible Executable Choke\n\n"
"## Goal\n"
"Keep the reversible successor orbit, but inject successor-pressure asymmetry inside the orbit itself.\n\n"
"## Change\n"
"Instead of toggling `00 C0 <-> 01 C0`, toggle the next cell's executed successor pair between:\n\n"
"- `00 C0` = `add al, al`\n"
"- `F7 D0` = `not eax`\n\n"
"The pair is still reversible, but now the two orbit phases are materially different, not a near-symmetric width-only change.\n\n"
"## Mechanism\n"
"Each cell executes:\n\n"
"- `BSR EAX,EAX`\n"
"- `SETZ DL`\n"
"- `LOCK XOR word ptr [next_mutable_pair], 0x10F7`\n"
"- mutable successor pair (`00 C0` or `F7 D0`)\n\n"
"## Measurements\n"
"- single-thread pass trace of the mutable pairs\n"
"- shared vs separate two-core latency\n"
"- odd/even parity means as a coarse orbit-phase split\n"
"- final mutable pair states\n");
    fclose(notes);

    trace_passes("/mnt/data/asymmetric_reversible_pass_trace.txt");
    ...
    FILE *sum = fopen("/mnt/data/asymmetric_reversible_exec_choke_summary.txt", "w");
    if (!sum) { perror("summary"); return 1; }
    fprintf(sum, "Asymmetric reversible executable choke summary\n");
    ...
    dump_pairs(sum, "shared_final_pairs:", &shared);
    dump_pairs(sum, "separate_A_final_pairs:", &sepA);
    dump_pairs(sum, "separate_B_final_pairs:", &sepB);
    fclose(sum);

    FILE *log = fopen("/mnt/data/test_asymmetric_reversible_exec_choke.log", "w");
    if (!log) { perror("log"); return 1; }
    fprintf(log, "Asymmetric reversible executable choke\n");
    fprintf(log, "Mutable pair: 00 C0 <-> F7 D0\n");
    ...
    dump_pairs(log, "shared_final_pairs:", &shared);
    dump_pairs(log, "separate_A_final_pairs:", &sepA);
    dump_pairs(log, "separate_B_final_pairs:", &sepB);
    fclose(log);

    write_csv("/mnt/data/asymmetric_reversible_shared_core0.csv", shared0);
    write_csv("/mnt/data/asymmetric_reversible_shared_core1.csv", shared1);
    write_csv("/mnt/data/asymmetric_reversible_separate_core0.csv", sep0);
    write_csv("/mnt/data/asymmetric_reversible_separate_core1.csv", sep1);
    ...
    dump_pairs(stdout, "shared_final_pairs:", &shared);
    dump_pairs(stdout, "separate_A_final_pairs:", &sepA);
    dump_pairs(stdout, "separate_B_final_pairs:", &sepB);
    return 0;
}
